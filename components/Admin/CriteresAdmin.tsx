...
interface CritereItem {
  ...
  first: { playerId?: string; equipeId?: string }[];
  second: { playerId?: string; equipeId?: string }[];
  third: { playerId?: string; equipeId?: string }[];
  status: boolean;
}
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setShowForm(false);
        setForm({ sessionId: '', designation: '', description: '', firstPoints: 0, secondPoints: 0, thirdPoints: 0, firstRecompense: '', secondRecompense: '', thirdRecompense: '' });
        await load();
      } else {
        ...
        <motion.form initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleCreate} className="rounded-2xl border border-stroke bg-white p-6 space-y-4 dark:border-strokedark dark:bg-blacksection">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="rounded-lg border border-amber-200 p-3 dark:border-amber-500/20">
              <p className="mb-2 text-xs font-medium text-amber-600">3e place</p>
              ...
              <input type="number" placeholder="Récompense $" value={form.thirdRecompense} onChange={(e) => setForm({ ...form, thirdRecompense: e.target.value })}
            ...
            <div className="rounded-lg border border-purple-200 p-3 dark:border-purple-500/20">
              <p className="mb-2 text-xs font-medium text-purple-600">2e place</p>
              <input type="number" placeholder="Points" value={form.secondPoints} onChange={(e) => setForm({ ...form, secondPoints: +e.target.value })}
                className="mb-2 w-full rounded-lg border border-stroke bg-transparent px-3 py-1.5 text-sm dark:border-strokedark" />
              <input type="number" placeholder="Récompense $" value={form.secondRecompense} onChange={(e) => setForm({ ...form, secondRecompense: e.target.value })}
                className="w-full rounded-lg border border-stroke bg-transparent px-3 py-1.5 text-sm dark:border-strokedark" />
            </div>
            <div className="rounded-lg border border-emerald-200 p-3 dark:border-emerald-500/20">
              <p className="mb-2 text-xs font-medium text-emerald-600">1re place</p>
              <input type="number" placeholder="Points" value={form.firstPoints} onChange={(e) => setForm({ ...form, firstPoints: +e.target.value })}
                className="mb-2 w-full rounded-lg border border-stroke bg-transparent px-3 py-1.5 text-sm dark:border-strokedark" />
              <input type="number" placeholder="Récompense $" value={form.firstRecompense} onChange={(e) => setForm({ ...form, firstRecompense: e.target.value })}
                className="w-full rounded-lg border border-stroke bg-transparent px-3 py-1.5 text-sm dark:border-strokedark" />
            </div>
          </div>
          ...
          <div key={c._id} className="rounded-2xl border border-stroke bg-white p-5 shadow-solid-5 dark:border-strokedark dark:bg-blacksection">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-waterloo">{(c.sessionId as any)?.designation || 'Session'} • {c.status ? 'Actif' : 'Inactif'}</p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-2 text-xs dark:border-amber-500/20 dark:bg-amber-900/10">
                <p className="font-medium text-amber-700 dark:text-amber-400">3e · {c.thirdPoints} pts</p>
                <p className="text-waterloo">${c.thirdRecompense} · {c.third?.length || 0} qualifié(s)</p>
              </div>
              <div className="rounded-lg border border-purple-200 bg-purple-50/50 p-2 text-xs dark:border-purple-500/20 dark:bg-purple-900/10">
                <p className="font-medium text-purple-700 dark:text-purple-400">2e · {c.secondPoints} pts</p>
                <p className="text-waterloo">${c.secondRecompense} · {c.second?.length || 0} qualifié(s)</p>
              </div>
              <div className="rounded-lg border border-emerald-200 bg-emerald-50/50 p-2 text-xs dark:border-emerald-500/20 dark:bg-emerald-900/10">
                <p className="font-medium text-emerald-700 dark:text-emerald-400">1re · {c.firstPoints} pts</p>
                <p className="text-waterloo">${c.firstRecompense} · {c.first?.length || 0} qualifié(s)</p>
              </div>
            </div>
          </div>
        ))}
